...
class ViewController: UIViewController {
    ...
    @IBAction func readSteps(_ sender: UIButton) {
        ...
        StepKitManager.shared.readSteps(months: 3, timeUnit: .day) { (success, records, error) in
            ...
            for stepRecord: DayRecord in records as? [DayRecord] ?? [DayRecord]() {
                print("\(stepRecord.startDate.description(with: .current)) to \(stepRecord.endDate.description(with: .current)) : steps = \(stepRecord.steps)")
            }
            
            if let records = records as? [DayRecord]  {
                if let todayRecord = records.first {
                    DispatchQueue.main.async {
                        self.todayStepLabel.text = "Today Step: \(todayRecord.steps)"
                    }
                }
            }
